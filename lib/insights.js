...
export function computeCoachAnalysis({ expenses, incomes, savingsTarget, now = new Date() }) {
  ...
  // How close to the target someone's current trajectory puts them — framed
  // around projection time rather than lifetime-accumulated leftover, since
  // the latter conflates "how long I've used this app" with "how good my
  // habits are." Someone saving well for 3 weeks should look like they're
  // doing well, not like they've barely started, just because the target is
  // large relative to a short history. We treat 3 years at the current pace
  // as "as far as the climb scales" — beyond that, more months don't make
  // the mountain meaningfully taller.
  let targetProgressPct = null;
  const HORIZON_MONTHS = 36;
  if (savingsTarget && savingsTarget > 0) {
    if (avgMonthlyLeftover > 0) {
      const monthsToGoal = savingsTarget / avgMonthlyLeftover;
      targetProgressPct = Math.max(0, Math.min(100, (1 - monthsToGoal / HORIZON_MONTHS) * 100));
    } else {
      targetProgressPct = 0;
    }
  }
...
/**
 * Suggests which category is most worth trimming, based on actual spending
 * share. Deliberately conservative: only suggests something if one category
 * clearly dominates (25%+ of total spending) — otherwise there's no honest
 * single answer to "where should I cut back," and saying nothing is better
 * than inventing a confident-sounding suggestion from thin evidence.
 */
export function suggestSpendingCut(avgCategoryMonthly, avgMonthlyExpense) {
  if (!avgCategoryMonthly || avgCategoryMonthly.length === 0 || avgMonthlyExpense <= 0) {
    return null;
  }
  const top = avgCategoryMonthly[0];
  const sharePct = (top.avgMonthly / avgMonthlyExpense) * 100;
  if (sharePct < 25) {
    return null;
  }
  return { key: top.key, sharePct: Math.round(sharePct), avgMonthly: top.avgMonthly };
}
